...
export class SomethingWentWrongComponent implements OnInit{
  ...
  onMouseMove(event: MouseEvent | TouchEvent) {
    ...
    const x = (eyes[0].getBoundingClientRect().left) + (eyes[0].getBoundingClientRect().width / 2);
    const y = (eyes[0].getBoundingClientRect().top) + (eyes[0].getBoundingClientRect().height / 2);
